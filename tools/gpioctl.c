				...
				const struct request *req)
{
	...
	if (chip) {
		for (i = 0; i < req->cfg.num_offsets; i++) {
			info = gpiod_chip_get_line_info(chip,
							req->cfg.offsets[i]);
			if (info) {
				  ...
				  struct request_opts *opts)
{
	...
	static const char *const shortopts = "+lp:hio";
...
static int request_main(int argc, char **argv, const struct config *cfg)
{
	...
	/* FIXME: multi-chip requests not supported */
...
{
	unsigned int num_infos, id, i, j, cnt;
				  ...
				  const char **fmt)
{
	...
	static const char *const shortopts = "+Fhn:";
...
{
	int num_events = 0, events_done = 0, timestamp_fmt = 0, sock;
	struct message msg = {
		.type = MSG_TYPE_MONITOR,
	};
	unsigned int num_infos, num_filter, id, i;
	struct resolved_name *resolved;
	struct request_info *infos;
	bool banner = false, filter;
	const char *fmt = NULL;
	...
	for (;;) {
		...
		print_event(msg.edge_ev.timestamp_ns, msg.edge_ev.offset,
			    msg.edge_ev.event_type, timestamp_fmt, fmt,
			    NULL, NULL, false);
		fflush(stdout);
...
static int reconfigure_main(int argc, char **argv, const struct config *cfg)
{
	...
	id = parse_request_id(argv[id_pos]);
...
static void parse_global_cmdline(int *argc, char ***argv, struct config *cfg)
{
	...
	static const char *const shortopts = "+hvc:sC:Bp:Q";
